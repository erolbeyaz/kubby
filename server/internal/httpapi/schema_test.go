...
// applySchema runs the project's migrations into an isolated schema, so integration
// tests exercise the real DDL rather than a hand-maintained copy that can drift.
	...
	for _, file := range files {
		...
		for _, stmt := range upStatements(string(raw)) {
			if _, err := db.Pool().Exec(ctx, stmt); err != nil {
				t.Fatalf("apply %s: %v\n%s", filepath.Base(file), err, stmt)
			}
		}
	}
}

// upStatements extracts the Up section of a goose migration.

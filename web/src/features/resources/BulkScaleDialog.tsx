...
export function BulkScaleDialog({
  ...
  const valid = mode === 'restore' || (Number.isInteger(replicas) && replicas >= 0)
  ...
  const run = async () => {
    ...
    for (const row of rows) {
      try {
        ...
        failed.push(`${row.namespace ?? ''}/${row.name}: ${caught instanceof ApiError ? caught.message : 'refused'}`)
      }
    }
  ...
  return (
    <ConfirmDialog
      ...
      confirmLabel={mode === 'restore' ? 'Restore' : replicas === 0 ? 'Scale to zero' : 'Scale'}
      busy={busy}
      disabled={!valid}
      destructive={mode === 'set' && replicas === 0}
      {...(failures.length > 0 ? { error: `${failures.length} of ${rows.length} were refused.` } : {})}
      onConfirm={() => void run()}
      onCancel={onClose}
    >
      <div className="flex flex-col gap-3 text-left">
        <div className="flex gap-1.5">
          <ModeTab label="Set to" active={mode === 'set'} onClick={() => setMode('set')} />
          <ModeTab label="Restore previous" active={mode === 'restore'} onClick={() => setMode('restore')} />
        </div>
        ...
        {mode === 'set' ? (
          <label className="flex items-center gap-3" style={{ fontSize: 'var(--text-secondary-size)' }}>
            ...
            <input
              value={wanted}
              onChange={(event) => setWanted(event.target.value.replace(/[^0-9]/g, ''))}
              inputMode="numeric"
              aria-label="Replicas"
              autoFocus
          ...
          <p style={{ fontSize: 'var(--text-micro)', color: 'var(--text-muted)' }}>
            Each workload goes back to the count it ran before Kubby last scaled it, read
            from the object itself. One that Kubby has not scaled has nothing recorded and
            will be reported below rather than guessed at.
          </p>
        )}
        ...
        >
          {rows.map((row, index) => (
            ...
            >
              <span className="min-w-0 flex-1 truncate font-mono" style={{ color: 'var(--text-secondary)' }}>
        ...
        {mode === 'set' && mixed && (
          <p style={{ fontSize: 'var(--text-micro)', color: 'var(--status-warn)' }}>
            These do not all run the same count. Kubby records what each one had, so
            Restore previous brings them back individually.
          </p>
        )}
        ...
        {failures.length > 0 && (
          <div className="flex flex-col gap-0.5" style={{ fontSize: 'var(--text-micro)' }}>
...
function currentReplicas(row: ResourceRow): number {
  ...
  const desired = ready.includes('/') ? Number(ready.split('/')[1]) : Number(row.fields['desired'])

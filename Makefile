...
#
#   make release VERSION=0.11.0 IMAGE_REGISTRY=localhost:5000
#   make release VERSION=0.11.0 IMAGE_REGISTRY=docker.io/erolbeyaz
...
release: ## Build a versioned image and push it. VERSION is required.
	@test "$(VERSION)" != "dev" || (echo "Set a version: make release VERSION=0.11.0"; exit 1)
...
tag: ## Tag the current commit as a release. VERSION is required.
	@test "$(VERSION)" != "dev" || (echo "Set a version: make tag VERSION=0.11.0"; exit 1)
